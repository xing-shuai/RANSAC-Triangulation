...
class RANSAC {
 private:
  ...
  std::vector<std::shared_ptr<T>>
      m_SampledModels;             // Vector of all sampled models
  std::shared_ptr<T> m_BestModel;  // Pointer to the best model, valid only
                                   // after Estimate() is called
  ...
  std::vector<std::mt19937> m_RandEngines;  // Mersenne twister high quality RNG
                                            // that support *OpenMP*
                                            // multi-threading
 ...
 public:
  ...
  void Reset(void) {
    // Clear sampled models, etc. and prepare for next call. Reset RANSAC
    // estimator state
    m_Data.clear();
  ...
  bool Estimate(const std::vector<std::shared_ptr<AbstractParameter>>& Data) {
    if (Data.size() <= t_NumParams) {
      std::cerr << "[ WARN ]: RANSAC - Number of data points is too less. Not "
                   "doing anything."
                << std::endl;
      return false;
    }
    ...
    std::uniform_int_distribution<int> UniDist(
        0, int(DataSize - 1));  // Both inclusive
...
#pragma omp parallel for
    for (int i = 0; i < m_MaxIterations; ++i) {
      ...
      std::vector<std::shared_ptr<AbstractParameter>> RandomSamples(
          t_NumParams);
      ...
      std::shuffle(RemainderSamples.begin(), RemainderSamples.end(),
                   m_RandEngines[omp_get_thread_num()]);  // To avoid picking
                                                          // the same element
                                                          // more than once
      std::copy(RemainderSamples.begin(),
                RemainderSamples.begin() + t_NumParams, RandomSamples.begin());
      // RemainderSamples.erase(RemainderSamples.begin(),
      // RemainderSamples.begin() + t_NumParams); // Remove the model data
      // points from consideration. 2018: Turns out this is not a good idea

      std::shared_ptr<T> RandomModel = std::make_shared<T>(RandomSamples);

      // Check if the sampled model is the best so far
      std::pair<VPFloat, std::vector<std::shared_ptr<AbstractParameter>>>
          EvalPair = RandomModel->Evaluate(RemainderSamples, m_Threshold);
